        ...
        try {
            for (Field field : clazz.getDeclaredFields()) {
                ...
                if (object instanceof String) {
                    ...
                    if (str.length() < validateInfo.min()){
                        String strErrors = String.format("%s must at least %s characters",columnInfor.columnName(),validateInfo.min());
                        bugs.add(strErrors);
                    }
                }
                ...
                if (object instanceof Integer) {
                    int min = (int) object;
                    if (min < validateInfo.min()){
                        String strErrors = String.format("%s must at least %s ",columnInfor.columnName(),validateInfo.min());
                        bugs.add(strErrors);
                    }
                }
                if (bugs.size() != 0){
